...
public class RecorderIncStatementGroovy1 extends AbstractRecorderIncStatement {
    ...
    public RecorderIncStatementGroovy1(final ClassNode clazz, final String recorderFieldName, final int index) {
    ...
    public void visit(org.objectweb.asm.MethodVisitor mv) {
        mv.visitFieldInsn(org.objectweb.asm.Opcodes.GETSTATIC,
                org.objectweb.asm.Type.getType("L" + ByteCodeUtilsGroovy1.getClassInternalName(clazz) + ";").getInternalName(),
                recorderFieldName,
                COVERAGE_RECORDER_DESC);
        ByteCodeUtilsGroovy1.pushConstant(mv, index);
    ...
    public void visit(groovyjarjarasm.asm.MethodVisitor mv) {
        mv.visitFieldInsn(groovyjarjarasm.asm.Opcodes.GETSTATIC,
                groovyjarjarasm.asm.Type.getType("L" + ByteCodeUtilsGroovy1.getClassInternalName(clazz) + ";").getInternalName(),
                recorderFieldName,
                COVERAGE_RECORDER_DESC);
        ByteCodeUtilsGroovy1.pushConstant(mv, index);
