...
public class CloverCompilationParticipant extends CompilationParticipant {
    ...
    public void buildStarting(BuildContext[] files, boolean isBatch) {
        if (files.length == 0) {
            ...
            logVerbose("----OPENCLOVER: BUILD ITERATION STARTED : " + new Date() + "----");
            try {
                ...
                logError("Unable to instrument and compile", e);
                try {
                    Markers.createCloverStaleDbMarker(
                        files[0].getFile().getProject(),
                        "OpenClover was unable to fully instrument and compile for this project. Please perform a clean rebuild. " + e.getMessage());
                } catch (CoreException e1) {
                    //Ignore. Look, we tried really hard to tell the user, give us a break.
                }
            }
            logVerbose("----OPENCLOVER: BUILD ITERATION ENDED : " + new Date() + "----");
        }
    ...
    public boolean isActive(IJavaProject project) {
        try {
            ...
            logError("Unable to determine if compilation participant is active for project " + project.getProject().getName(), e);
            return false;
        }
    }
}
